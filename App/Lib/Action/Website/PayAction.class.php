...
class PayAction extends HCommonAction {
	...
	public function _Myinit(){
		...
		$this->member_url = "http://".$_SERVER['HTTP_HOST']."/user";
		$this->payConfig = FS("Dynamic/payconfig");
	...
	public function offline(){
		...
		$this->paydetail['off_bank'] = $config['BANK'][$bank_id]['bank'].' 开户名：'.$config['BANK'][$bank_id]['payee'].'客户银行卡号:'.$_POST['bank_card'];
		$this->paydetail['off_way'] = text($_POST['off_way']);
        $pl = new PaylogModel();
		$newid = $pl->add($this->paydetail);
		if($newid){
			$this->success("线下充值提交成功，请等待管理员审核","/user");
		}else{
	...
	//国付宝接口
	 public function guofubaopay(){
		...
		$submitdata['goodsName'] = $this->glo['web_name']."账户充值";
		...
		$submitdata['virCardNoIn'] = $this->payConfig['guofubao']['virCardNoIn'];//国付宝账户
	...
	public function chinabank(){
		if($this->payConfig['chinabank']['enable']==0) exit("对不起，该支付方式被关闭，暂时不能使用!");
                ...
                // 支付信息
		$this->getPaydetail();
		$submitdata['v_mid'] = $this->payConfig['chinabank']['mid']; // 商户编号
		$submitdata['v_oid'] = date( "YmdHis" ).mt_rand( 100000, 999999 ); // 订单编号
		$submitdata['v_amount'] = $this->paydetail['money']; // 订单总金额
		$submitdata['v_moneytype'] = 'CNY'; // 币种
		$submitdata['v_url'] = $this->return_url."?payid=chinabank"; // 商户自定义返回接收支付结果的页面
		if($this->paydetail['bank']){ 
			$submitdata['pmode_id'] = $this->paydetail['bank']; // 银行直联必须
		}
		$submitdata['remark1'] ='';
		$submitdata['remark2'] ='[url:='.$this->notice_url."?payid=chinabank".']'; // 服务器异步通知的接收地址。对应AutoReceive.php示例。必须要有[url:=http://domain/chinabank/AutoReceive.php]格式。
		...
		$this->paydetail['way'] = 'chinabank';
                $this->paydetail['tran_id'] = $submitdata['v_oid'];
		M('member_payonline')->add($this->paydetail);
                
		$this->create($submitdata,"https://tmapi.jdpay.com/PayGate"); // 正式环境
	}
	...
	public function baofoo(){
		...
		$submitdata['ProductName'] = urlencode($this->glo['web_name']."账户充值" );
		$submitdata['Amount'] = "1";
		$submitdata['Username'] = "";
	...
	public function shengpay(){
		...
		$submitdata['ProductName'] = $this->glo['web_name']."账户充值";
	...
	public function ecpss(){
		...
		$submitdata['products'] = $this->glo['web_name']."账户充值";// '------------------物品信息
		//////////////////////////////////////////////////////////////////
	...
	public function allinpay()
	{
        ...
        $this->paydetail['tran_id'] = $submitdata['orderNo'];
        $pl = new PaylogModel();
        $res = $pl->addPayLog($this->paydetail);
        if($res){
//            $this->create($submitdata,"http://ceshi.allinpay.com/gateway/index.do" );//测试环境
        ...
        //通联wap支付
	public function allinpaywap()
	{
                $wappay_config = FS("Dynamic/wappayconfig");
                $allinppaywap_config = $wappay_config['allinpaywap'];
		if ($allinppaywap_config['enable'] == 0){
			exit( "对不起，该支付方式被关闭，暂时不能使用!" );
		}
		$this->getPaydetail();
		$submitdata['version'] = "v1.0";
		...
		$submitdata['issuerId'] = "";
		$submitdata['pan'] = "";
		$submitdata['merchantId'] = $allinppaywap_config['merchantId'];
		$submitdata['orderDatetime'] = date( "YmdHis" );
		$submitdata['pickupUrl'] = $this->return_url."?payid=allinpaywap";
		$submitdata['receiveUrl'] = $this->notice_url."?payid=allinpaywap";
		$submitdata['signMsg'] = $this->getSign( "allinpaywap", $submitdata );
        $this->paydetail['fee'] = getfloatvalue( $this->payConfig['allinpay']['feerate'] * $this->paydetail['money'] / 100, 2 );
        $this->paydetail['way'] = "allinpaywap";
        $this->paydetail['tran_id'] = $submitdata['orderNo'];
        $pl = new PaylogModel();
        $res = $pl->addPayLog($this->paydetail);
        if($res){
            $this->create($submitdata,"http://ceshi.allinpay.com/mobilepayment/mobile/SaveMchtOrderServlet.action" );//测试环境
//		    $this->create($submitdata,"http://service.allinpay.com/gateway/index.do" );//生产环境
        }else{
            exit( "系统繁忙请重试" );
        }
	...
	public function payreturn(){
			...
			case 'gfb':
				...
				if($recode=="0000"){//充值成功
					...
					if($_REQUEST['signValue']==$signGet){//充值完成
						$this->success("充值完成","/user/");
					}else{//签名不付
						$this->error("签名不付","/user/");
					}
				}else{//充值失败
						$this->error(auto_charset($_REQUEST['msgExt']),"/user/");
				}
			break;
			case "ips" :
		...
		case "baofoo" :
			...
			if($recode == "1"){
				...
				$this->error(auto_charset($_REQUEST['resultDesc']), "/user/" );
			}
		break;
		...
		break;
			...
			if ( $payResult == "1"){
				$signGet = $this->getSign( "allinpay_return", $_REQUEST );
				if ($signGet){
					$this->success( "充值完成", "/M/User/" );
				}else{
					$this->error( "签名不付", "/M/User/" );
				}
			}else{
				$this->error( "充值失败","/M/User/" );
			}
		break;
		}
	}
		...
		switch($payid){
			case 'gfb':
				...
				if($recode=="0000"){//充值成功
					...
					if($_REQUEST['signValue']==$signGet){//充值完成
						$done = $this->payDone(1,$_REQUEST['orderId']);
					}else{//签名不付
						$done = $this->payDone(2,$_REQUEST['orderId']);
					}
				}else{//充值失败
					$done = $this->payDone(3,$_REQUEST['orderId']);
				}
			...
			case 'chinabank':
				...
				if($v_pstatus=='20'){
					...
					if($_REQUEST['v_md5str'] == $signGet){
                                                // 充值完成
						$done = $this->payDone(1,$_REQUEST['v_oid']);
					}else{
                                                // 签名不符
						$done = $this->payDone(2,$_REQUEST['v_oid']);
					}
				}else{
                                        // 充值失败
					$done = $this->payDone(3,$_REQUEST['v_oid']);
				}
			...
			case "baofoo" :
				...
				if ( $recode == "1" )
				{
					...
					$nid = $this->createnid( "baofoo", $_REQUEST['TransID'] );
					if ($_REQUEST['Md5Sign'] == $signGet){
						$done = $this->payDone(1,$_REQUEST['TransID']);
					}else{
						$done = $this->payDone(2,$_REQUEST['TransID']);
					}
				}else{
					$done = $this->payDone(3,$_REQUEST['TransID']);
				}
				if($done===true) echo "OK";
				else echo "Fail";
			break;
			case "shengpay" :
				...
				if ( $recode == "01" )
				{
					$signGet = $this->getSign( "shengpay_return", $_REQUEST );
					if ($_REQUEST['SignMsg'] == $signGet){
						$done = $this->payDone(1,$_REQUEST['OrderNo']);
					}else{
						$done = $this->payDone(2,$_REQUEST['OrderNo']);
					}
				}else{
					$done = $this->payDone(3,$_REQUEST['OrderNo']);
				}
				if($done === true){
					echo "OK";
				}else{
			...
			case "ecpss":
				...
				if(strtoupper($_REQUEST['SignMD5info']) == $signGet){
					...
					//if ($recode=="1" || $recode=="9" || $recode=="19" || $recode=="88") {
					if ($recode=="88") {
						$done = $this->payDone(1,$_REQUEST['BillNo']);
					}else{
						$done = $this->payDone(2,$_REQUEST['BillNo']);
					}
				}else{
					$done = $this->payDone(3,$_REQUEST['BillNo']);
				}
			break;
			case "tenpay":
				$recode = $_REQUEST['trade_state'];
				if ($recode == "0"){
					...
					if ( strtolower( $_REQUEST['sign']) == $signGet ){
						$done = $this->payDone(1,$_REQUEST['transaction_id']);
					}else{
						$done = $this->payDone(2,$_REQUEST['transaction_id']);
					}
				}else{
					$done = $this->payDone(3,$_REQUEST['transaction_id']);
				}
				if($done === true){
					echo "success";
				}else{
		...
		public function payipsnotice(){
			$recode = $_REQUEST['succ'];
				if ( $recode == "Y" )
				{
					$signGet = $this->getSign( "ips_return", $_REQUEST );
					if ( $_REQUEST['signature'] == $signGet ){
						$done = $this->payDone( 1, $_REQUEST['ipsbillno'] );
					}else{
						$done = $this->payDone( 2, $_REQUEST['ipsbillno'] );
							echo "签名不正确";
					}
				}else{
					$done = $this->payDone( 3, $_REQUEST['ipsbillno']  );
				}
				if ( $done === true ){
		...
		if(empty($_POST)){//判断提交来的数组是否为空
			...
			if($signGet==$_POST["sign"]){
				...
				if ($recode=="TRADE_FINISHED") {
					...
					$this->error( "交易失败", "/user/" );
				}
			}else{
				//验证失败的处理
				$this->error("数字签名不符".$_POST["sign"], "/user/" );
			}
		}
	}
	//易生支付返回服务器端处理
	public function payeasypaynotice(){
		if(empty($_POST)){//判断提交来的数组是否为空
			...
			if($signGet==$_POST["sign"]){
				$recode = $_POST['trade_status'];
				if($recode == "TRADE_FINISHED"){
					$done = $this->payDone( 1,$_POST['out_trade_no']);
				}else{
					$done = $this->payDone( 2, $_POST['out_trade_no']);
				}
			}else{
				$done = $this->payDone(3,$_POST['out_trade_no']);
			}
			if ( $done === true ){
...
////////////////////////////////////////////易生支付接口返回处理方法结束	fan20140114/////////////////////////////
	...
	private function payDone($status,$oid){
		if($this->locked) return false;
		$this->locked = true;
		$pl = new PaylogModel();
        $res = $pl->payDone($oid,$status);
		$this->locked = false;
		return $res;
	}
	...
	private function getSign($type,$data){
		...
		switch($type){
			...
			return $md5str;
				...
				$md5str .= "&key=".$allinppaywap_config['pkey'];
				$md5str = strtoupper(md5($md5str));
			return $md5str;
