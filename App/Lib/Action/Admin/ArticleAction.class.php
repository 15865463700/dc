...
class ArticleAction extends ACommonAction
{
    ...
    */
    ...
    {
		$tlist = M("article a")->field("ac.type_name,a.type_id,ac.parent_id")->join("{$this->pre}article_category ac ON ac.id=a.type_id ")->group("a.type_id")->limit(6)->select();
    ...
    {
		$typelist = get_type_leve_list('0','acategory', 'article');//分级栏目
	...
	public function _doEditFilter($m){
		...
		if($data['art_img']) $m->art_img=$data['art_img'];
		if($_POST['is_remote']==1) $m->art_content = get_remote_img($m->art_content);
		alogs("article",0,1,'成功执行了文章编辑操作！');//管理员操作日志
		return $m;
	}
