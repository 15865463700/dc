...
class PaylogAction extends ACommonAction
{
    ...
    */
	...
	public function doEdit(){
		...
		if($status==1){
			$vo = M('member_payonline')->field('money,fee,uid,way,add_time')->find($id);
            $newid = $mm->updateMemberMoney($vo['uid'],3,$vo['money']-$vo['fee'],"管理员手动审核充值");
			//var_dump($vo);var_dump($newid);exit;
			if($newid){
				...
				if($vo['way']=="off"){
					...
					$newidx = $mm->updateMemberMoney($vo['uid'],56,$vo['money']*$fee_rate,"线下充值奖励");
				}
				/////////////////////////////
				/*
				...
				if($vo['way']=="off") SMStip("payoffline",$vx['user_phone'],array("#USERANEM#","#DATE#","#MONEY#"),array($vx['user_name'],date('Y-m-d H:i:s',$vo['add_time']),$vo['money']));
				else  SMStip("payonline",$vx['user_phone'],array("#USERANEM#","#DATE#","#MONEY#"),array($vx['user_name'],date('Y-m-d H:i:s',$vo['add_time']),$vo['money']));
				alogs("Paylog",0,1,'执行了管理员手动审核充值操作！');//管理员操作日志
				$this->success("处理成功");
			}else{
				alogs("Paylog",0,0,'执行管理员手动审核充值操作失败！');//管理员操作日志
				$this->error("处理失败");
			}
		}else{
			$save['deal_user'] = session('adminname');
			...
			if($newid){
				...
				alogs("Paylog",0,0,'执行了管理员手动审核充值操作失败！');//管理员操作日志
				$this->error("处理失败");
			}
		}
	}
