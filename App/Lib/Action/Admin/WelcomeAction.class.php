...
class WelcomeAction extends ACommonAction {
    ...
    public function index(){
        ...
        $row['debt_a'] = M('invest_debt')->where('status=99')->count('id');//债权转让待审核
       ...
       //逾期
		...
		/////////////////////////////////////////////////////////////
		$sql_chart_1 = "select count(x.t) as e  from (select count(*) as t from shang_borrow_info group by borrow_uid) as x";
		$chart1_borrow = M()->query($sql_chart_1);
		$memberCount = M("members")->count("*");
		$sql_chart_3 = "select count(x.t) as e  from (select count(*) as t from shang_borrow_investor group by investor_uid) as x";
