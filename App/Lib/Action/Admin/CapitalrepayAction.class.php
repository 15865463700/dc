...
//		$map['i.progress'] = 100;
    ...
    public function san()
    {
        ...
        $where['group'] = "d.borrow_id,d.sort_order";
        $where['order'] = "d.deadline asc";
        $field = "d.borrow_id,m.user_name,b.borrow_name,sum(d.capital) as bmoney,sum(d.interest) as interest,
        sum(d.capital+d.interest) as total_money,d.sort_order,b.total,d.deadline";
    ...
    public function exportsan(){
        ...
        $row=array();
        $row[0]=array('序号','标号ID','用户名','借款标题','待还期数/总期数','待还本金','待还利息','总待还金额','待还时间');
        $i=1;
        foreach($list as $v){
            $row[$i]['i'] = $i;
            $row[$i]['borrow_id'] = $v['borrow_id'];
            $row[$i]['user_name'] = $v['user_name'];
            $row[$i]['borrow_name'] = $v['borrow_name'];
            $row[$i]['sort'] = $v['sort_order']."/".$v['total'];
            $row[$i]['bmoney'] = $v['bmoney'];
            $row[$i]['interest'] = $v['interest'];
