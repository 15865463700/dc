...
//            exit;
            ...
            $btname = M('members')->where("id=".$rid)->find();
            alogs("spread",0,1,'成功执行了关联的操作！推荐人:'.$name.'被推荐人:'.$btname['user_name']);//管理员操作日志
            if($rs !== false){
                $sp = new SpreadModel();

                $sp_data['uid'] = $rid;
                $sp_data['rid'] = $id;
                $count = $sp->where($sp_data)->count();
                if(!$count){
...
//        $uid = $_GET['uid'];
        ...
        $tjname=M('members')->where("id=".$m['recommend_id'])->find();
        if(!$m){
            echo 1;
            exit;
        }
...
//        exit;
        ...
        alogs("spread",0,1,'成功执行了取消关联的操作！取消的推荐人:'.$tjname['user_name'].'被推荐人:'.$m['user_name']);//管理员操作日志
        if($rs !== false){
            echo 2;
            exit;
        }else{
