...
class VerifyphoneAction extends ACommonAction
{
    ...
    */
    public function index()
    {
		...
		$field= 'm.id,m.user_phone,m.reg_time,m.user_name,mi.real_name,
		mm.money_freeze,mm.money_collect,(mm.account_money+mm.back_money) account_money,
		ms.phone_status,mi.uid';
        $where['map'] = $map;
        $where['limit'] = C('ADMIN_PAGE_SIZE');

        $join[] = "__MEMBER_MONEY__ mm ON ms.uid=mm.uid";
        $join[] = "__MEMBER_INFO__ mi ON ms.uid=mi.uid";
        $join[] = "__MEMBERS__ m ON ms.uid=m.id";
...
////////////////////////////添加手动手机验证  fan 2013-11-27(手动验证已不需要) ///////////////////////////////////////
