...
class MsgonlineAction extends ACommonAction
{
    ...
    */
    public function index()
    {
        ...
        $uid6=$msgconfig['sms']['user6']; //分配给你的账号
        $pwd6=$msgconfig['sms']['pass6']; //密码
		if($type==0){
            ...
            $post_data['userid'] = $uid5;
            $post_data['account'] = $account5;
            $post_data['password'] = $pwd5;
            $post_data['json'] = 1;
            $url='http://client.movek.net:8888/sms.aspx?action=overage';
            //$d = @file_get_contents("http://218.244.136.70:8888/sms.aspx?action=overage&userid={$uid4}&account={$account4}&password={$pwd4}",false);
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_POST, 1);
            curl_setopt($ch, CURLOPT_HEADER, 0);
            ...
            $remain = $data['data']['overage'];
            $this->assign("mdsh", $remain);
        }else{
       ...
       //取出短信群
		...
		$this->assign("type_list", array("3"=>'关闭短信平台服务',"1"=>'漫道短信提供商',"2"=>'亿美软通短信提供商',"4"=>"漫道二次接口","5"=>"漫道三次接口","6"=>"微网通联接口(还没接，不能用)"));
        $this->display();
    }
