...
class MemberidAction extends ACommonAction
{
    ...
    */
    public function index()
    {
		$map=array();
		if($_REQUEST['status']){
			$map['ms.id_status'] = intval($_REQUEST['status']);
			$search['status'] = intval($_REQUEST['status']);
		}
		else  $map['ms.id_status'] = array("in","1,3");
		...
		if($_REQUEST['realname']){
			...
			$search['real_name'] = urldecode($_REQUEST['realname']);
		}
        ...
        $field = "ms.uid,ms.id_status,mi.up_time,mi.card_img,mi.card_back_img,mi.real_name,
            mi.idcard,m.user_name";
        $join[] = "__MEMBERS__ m ON m.id=ms.uid";
        $join[] = "__MEMBER_INFO__ mi ON mi.uid=ms.uid";
        $where['map'] = $map;
        $where['limit'] = C('ADMIN_PAGE_SIZE');
        $where['order'] = 'mi.up_time DESC';
        $list = $ms->getList($field,$where,$join);
	...
	public function export(){
		import("ORG.Io.Excel");
		alogs("Memberid",0,1,'执行了实名认证会员列表导出操作！');//管理员操作日志
		$pre = C('DB_PREFIX');
	...
	public function doEdit(){
        ...
        $m = new MembersModel();
        $updata["NameApply"] = $data;
        $new = $m->updateMemberInfo($uid,$updata);
		if($newxid&&$new){
            M()->commit();
			alogs("Memberid",$newxid,1,'成功执行了会员实名认证的操作！备注信息：'.$deal_info);//管理员操作日志
			$this->success("审核成功",__URL__."/index".session('listaction'));
		}else{
            M()->rollback();
			alogs("Memberid",$newxid,0,'执行会员实名认证的操作失败！备注信息：'.$deal_info);//管理员操作日志
			$this->error("审核失败");
		}
	}
