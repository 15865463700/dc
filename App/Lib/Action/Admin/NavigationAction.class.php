<?php
// 全局设置
class NavigationAction extends ACommonAction
{
	var $typeleve=1;
    ...
    */
    ...
    {
        $field= true;
        $this->_list(D('navigation'),$field,array("parent_id"=>0, 'model'=>'navigation'),'sort_order');
        $this->display();
    }
	...
	public function _doEditFilter($m){
		$m->parent_id=intval($m->parent_id);
		$m->type_set=2;
		alogs("navigation",0,1,'成功执行了导航菜单编辑操作！');//管理员操作日志
		return $m;
	}
    ...
    public function listType()
    {
		...
		$sonlist = D('navigation')->field(true)->where("parent_id={$typeid}")->select();
