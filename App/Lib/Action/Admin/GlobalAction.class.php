    ...
    */
    ...
    public function addFriend()
    {
		...
		if(!isset($_POST['fid'])){//新增
			...
			alogs("friend",0,1,'成功执行了添加友情链接操作！');//管理员操作日志
			...
			alogs("friend",0,1,'成功执行了编辑友情链接操作！');//管理员操作日志
			$this->assign('jumpUrl', U('/admin/global/friend/'));
    ...
    public function doDeleteFriend()
    {
		...
		if($delnum){
			$a_data['success'] = $rid;
			alogs("friend",0,1,'成功执行了删除友情链接操作！');//管理员操作日志
			$a_data['success_msg'] = "友情链接删除成功";
			$a_data['aid'] = $idarray;
		}else{
			$a_data['success'] = 0;
			alogs("friend",0,0,'友情链接删除操作失败！');//管理员操作日志
    ...
    public function cleantemplet()
    {
		...
		if($_GET['acahe']==1){//前台
			...
			$dirs	=	array(C('APP_ROOT').'Runtime/Cache/User');
		}else{
			exit("ERROR");
		}
		foreach($dirs as $value)
	...
	public function admintjlog(){
		...
		if(!empty($_REQUEST['start_time'])&& !empty($_REQUEST['end_time'])){
			...
			$start_time = strtotime($_REQUEST['start_time']);
			$map['l.deal_time'] = array("gt",$start_time);
			$search['start_time'] =$start_time;
		}elseif(!empty($_REQUEST['end_time'])){
			$end_time = strtotime($_REQUEST['end_time']);
			$map['l.deal_time'] =array("lt",$end_time);
			$search['end_time'] = $end_time;
		}
		//分页处理
		import("ORG.Util.Page");
		$count = M('ausertj_dologs l')->where($map)->count('l.id');
		$p = new Page($count, C('ADMIN_PAGE_SIZE'));
		$page = $p->show();
		$Lsql = "{$p->firstRow},{$p->listRows}";

		$list = M('ausertj_dologs l')->field(true)->where($map)->order("l.id DESC")->limit($Lsql)->select();
		$this->assign("list", $list);
		$this->assign("pagebar", $page);
		$this->assign("search", $search);
		$this->assign("query", http_build_query($search));
	...
	public function dodeletelog()
	{
		...
		if($delnum){
			$a_data['success'] = $rid;
			alogs("auser_dologs",0,1,'成功执行了后台操作日志删除操作！');//管理员操作日志
			$a_data['success_msg'] = "后台操作日志删除成功";
			$a_data['aid'] = $idarray;
		}else{
			$a_data['success'] = 0;
			alogs("auser_dologs",0,0,'后台操作日志删除失败操作！');//管理员操作日志
	...
	{
		$map=array();
		$end = strtotime("-1 month",strtotime(date("Y-m-d",time())." 00:00:00"));
		$map['deal_time'] = array("lt",$end);
    ...
    public function bidName(){
		...
		alogs("bid_name",0,1,'成功执行了标名设置操作！');//管理员操作日志
        $this->success("修改成功！");
    }
