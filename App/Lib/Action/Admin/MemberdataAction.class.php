...
class MemberdataAction extends ACommonAction{
    public function index(){
		...
		$list = M('member_data_info mf')->field('mf.*,au.user_name as real_name,m.user_name as uname,m.customer_name')->join("{$this->pre}members m ON m.id=mf.uid")->join("{$this->pre}ausers au ON au.id=mf.deal_user")->where($map)->limit($Lsql)->order("mf.id DESC")->select();
	...
	public function doEdit(){
        ...
        if ($result = M('member_data_info')->save($vd)) { //保存成功
            ...
            //发送站内信
            if($vd['status']==1){
                $content = '恭喜，您的资料（'.$vd['data_name'].'）审核通过![网贷系统V5]';
                addInnerMsg($vd['uid'], '资料审核通过', $content);
            }else{
                $content = '对不起，您的资料（'.$vd['data_name'].'）审核失败!审核说明：'.$vd['deal_info'].'[网贷系统V5]';
                addInnerMsg($vd['uid'], '资料审核失败', $content);
            }            
            //成功提示
            $this->assign('jumpUrl');
            $this->success(L('修改成功'));
	...
	public function uploadshow(){
		...
		$list = M("member_borrow_show")->where("uid=$uid")->order("sort DESC")->select();
		$this->assign('vo',$user);
		$this->assign('list',$list);
        $this->display();
	}
