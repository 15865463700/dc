...
class ACommonAction extends Action
{
	...
	function _initialize(){
        ...
        $this->justlogin = !isset($this->justlogin)?false:$this->justlogin;
		...
		if(session('admin')){
            ...
            $codecheck=$this->datag['admin_url'];
            $code = $_GET['code'];
            if($code!=$codecheck){
                redirect(C('ERROR_PAGE'));
                exit;
            }
			redirect("/admin/index/login?code={$code}");
			exit;
		}
    ...
    public function doEdit() {
        ...
        //保存当前数据对象
        if ($result = $model->save()!==false) { //保存成功
	...
	public function memberheaderuplad(){
		$uid = $_GET['uid'];
		if(empty($uid)) exit;
		else redirect("/Style/header/upload.php?uid={$uid}");
		exit;
	}
