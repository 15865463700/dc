...
class AgreementAction extends MCommonAction {
 ...
 public function downfile(){
	 ...
	 public function downliuzhuanfile(){
		...
		$this->assign("bid","RZHT-L-".str_repeat("0",5-strlen($binfo['id'])).$binfo['id']);
 ...
 public function downdingtoubao(){
		...
		$this->assign("bid","RZHT-U-".$iinfo['borrow_id'].str_repeat("0",6-strlen($iinfo['id'])).$iinfo['id']);
    ...
    public function download(){
        ...
        $per = C('DB_PREFIX');
        $invest_id=intval($_GET['id']);
        $iinfo = M('borrow_investor')
            ->field('id,borrow_id,investor_capital,investor_interest,deadline,investor_uid,add_time')
            ->where("(investor_uid={$this->uid} OR borrow_uid={$this->uid}) AND id={$invest_id}")->find();
        $binfo = M('borrow_info b')->field('b.id,b.repayment_type,b.borrow_duration,b.borrow_uid,
        b.borrow_type,b.borrow_use,b.borrow_money,b.full_time,b.add_time,b.borrow_interest_rate,
        b.deadline,b.second_verify_time')
            ->where("b.id = {$iinfo['borrow_id']}")->find();
        ...
        $mBorrow = M("members m")->join("{$per}member_info mi ON mi.uid=m.id")
            ->field('mi.real_name,m.user_name')->where("m.id={$binfo['borrow_uid']}")->find();
        $mInvest = M("members m")->join("{$per}member_info mi ON mi.uid=m.id")
            ->field('mi.real_name,m.user_name,m.user_phone')->where("m.id={$iinfo['investor_uid']}")->find();
        if(!is_array($iinfo)||!is_array($binfo)||!is_array($mBorrow)||!is_array($mInvest)) exit;
        ...
        $type = $this->gloconf['REPAYMENT_TYPE'];
        //echo $binfo['repayment_type'];
        $binfo['repayment_name'] = $type[$binfo['repayment_type']];
        ...
        $pdf->SetCreator($datag['web_name']);
        $pdf->SetAuthor($datag['web_name']);
        $pdf->SetTitle($datag['web_name']);
        $pdf->SetSubject($datag['web_name']);
        $pdf->SetKeywords($datag['web_name']);
...
<div style="width:850px;">
    ...
    <div style="color: #4c4948;font-size: 30px;font-weight: normal;padding-left:260px;">
        ...
        <table style="width:100%;" border="0">
            ...
            .'<br/><br/>甲方（借款人）：（本站ID：'.hidecard($mBorrow["user_name"])
            .'）<br/><br/>丙方（投资人）：详见本协议第一条<br/><br/>丁方（居间方）：'.$ht["name"]
...
<p>&nbsp;</p>
    ...
    public function downloaddingtoubao(){
        ...
        $bid = "RZHT-U-".$iinfo['borrow_id'].str_repeat("0",6-strlen($iinfo['id'])).$iinfo['id'];
        $type1 = $this->gloconf['BORROW_USE'];
        $binfo['borrow_use'] = $type1[$binfo['borrow_use']];
...
//
        ...
        $html .='<p>尊敬的客户：<br/>';
        $datag['web_name'];
        $html .= '优计划（以下称"优计划"）旨在为投资理财用户提供效率更高、退出更加灵活、资金利用率更高的服务，以便更好地服务于出借人。
                </p>';
...
//        {$ht.name|default="未填写"}
        if($mBorrow["real_name"]){
            $html .= $mBorrow["real_name"];
        }else{
            $html .= "未填写";
        }
    ...
    <p>咨询电话：<span style="text-decoration:underline">&nbsp;';
        if($mBorrow["cell_phone"]){
            $html .= $mBorrow["cell_phone"];
        }else{
            $html .= "未填写";
        }
	...
	<p>&nbsp; 1.2&nbsp;甲方同意加入本次优计划计划，并自愿遵守网贷系统V5网站的所有规则。</p>
	<br />
	<p>2、优计划金额</p>
	<p>&nbsp; 2.1&nbsp;申请加入时间：'.date("Y-m-d",$iinfo['add_time']).'  加入金额（单位:元）：   '.$list3[0]["investor_capital"].'   </p>
	<p>&nbsp; 2.2&nbsp;甲方加入本次优计划计划后，乙方将按照甲方首次加入计划的时间先后顺序，将甲方加入计划的金额在网贷系统V5P2C平台安排进入优先自动投标。</p>
...
<p>&nbsp;3.11 &nbsp; 甲方收益及支付：甲方的收益将按本次优计划计划所投标的对应的《借款协议》分别确认。根据所投项目的还款周期，优计划计划所投标的的利息收入将根据用户的选择（用户有三种选择：a.提现到银行卡，b.转入出借人在网贷系统V5平台的账户，c.利息复投），如果选择提现到银行卡则自动提取至甲方指定账户；如果选择转入到出借人自己的网贷系统V5账户，则利息按期自动转入；如果选择利息复投则系统会自动在优计划计划内自动投标。选择退出计划后，转让本次计划项下所有债权后的收益，也将提取至指定账户。</p>
...
<p>&nbsp;8.1 &nbsp; 本次优计划计划不提供纸质账单。甲方需要通过网贷系统V5网站或客服人员、网站平台查询等方式了解相关信息。如未及时查询，或由于通讯故障、系统故障以及其他不可抗力等因素影响使甲方无法及时了解相关信息，由此产生的责任和风险由甲方自行承担。</p>

<p>&nbsp;8.2 &nbsp; 本次优计划计划并非独立于甲方在注册、使用网贷系统V5网站时需要遵守的相应规则和订立的相关协议。甲方必须遵守网贷系统V5的相关规则和签署的相关协议，如违背的，将自行承担相应后果。</p>

<p>&nbsp;8.3 &nbsp; 由于地震、火灾、战争等不可抗力导致的交易中断、延误的，甲乙双方互不追究责任。但应在条件允许的情况下，应采取一切必要的补救措施以减小不可抗力造成的损失。</p>

<p>&nbsp;8.4 &nbsp; 本次优计划计划金额未达到优计划计划规模要求，经网贷系统V5合理判断难以按照优计划计划协议约定向甲方提供相应服务的，网贷系统V5有权宣布优计划计划不成立。</p>
