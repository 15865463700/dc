<?php
// 全局设置
class MVCommonAction extends Action
{
	var $glo=NULL;
	var $uid=0;
	//上传参数
	...
	protected function _initialize()
    {
        ...
        if ($this->notneedlogin === true) {
            if (session("u_id")) {
                ...
                if (!in_array(strtolower(ACTION_NAME), array("actlogout", 'regsuccess', 'emailverify', 'verify'))) redirect("/user/");
	...
	public function memberheaderuplad(){
		...
		else redirect("/Style/header/upload.php?uid={$this->uid}");
		exit;
	}
	//上传图片
	...
	protected function _memberlogin($uid){
			$vo = M('members')->field("id,user_name,recommend_id")->find($uid);
			if(is_array($vo)){
				foreach($vo as $key=>$v){
					session("u_{$key}",$v);
				}

                $res1 = M('members')->field('second_teamid,first_teamid')->where(' id = '.$vo['id'])->find();

                if($res1['second_teamid'] != 0 || $res1['first_teamid'] != 0){
                    session("u_recommend",'(推广团队成员)');
                }else{
                    session("u_recommend",'(会员)');
                }
