...
class MCommonAction extends Action
{
	...
	protected function _initialize(){
        ...
        $qqlist = M("qq")->where(array("is_show"=>1,"type"=>0))->limit(3)->order("qq_order desc")->select();
		...
		if($this->notneedlogin === true){
			if(session("u_id")){
                ...
                if(!in_array(strtolower(ACTION_NAME),array("actlogout"))&&!in_array(strtolower(MODULE_NAME),array("register","common","login"))){
                    redirect("/User");
                    exit;
                }
			}
		}elseif(session("u_user_name")){
            if(session("u_id")&&strtolower(ACTION_NAME)!="actlogout"&&$_REQUEST['_URL_'][0]!='register') {
                $this->uid = session("u_id");
                $m = new MembersModel();
                $checkinfo = $m->getMemberInfo($this->uid,'Verify',"id,pin_pass");
