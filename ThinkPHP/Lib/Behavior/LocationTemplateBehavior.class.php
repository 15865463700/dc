...
class LocationTemplateBehavior extends Behavior {
    ...
    private function parseTemplateFile($templateFile) {
        if(''==$templateFile) {
            ...
            // 解析规则为 模板主题:模块:操作 不支持 跨项目
            // 新增跨分组调用规则，例：   M::User:returnmsg分组名使用两个冒号分割
            $pathX = explode('::',$templateFile);
            if(empty($pathX[1])){
                $path   =  explode(':',$templateFile);
            }else{
                $path   =  explode(':',$pathX[1]);
            }
            ...
            if(!empty($path)) {// 设置模板主题
                ...
                if(!empty($pathX[1])){
                    $path = TMPL_PATH.$pathX[0].'/'.(THEME_NAME?THEME_NAME:'').'/'.array_pop($path).'/';
                }
            }else{
                $path = THEME_PATH;
                if(!empty($pathX[1])){
                    $path = TMPL_PATH.$pathX[0].'/'.(THEME_NAME?THEME_NAME:'').'/';
                }
            }
