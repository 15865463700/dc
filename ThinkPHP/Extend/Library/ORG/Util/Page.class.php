...
class Page {
    ...
    protected $coolPages   ;
    // 分页显示定制
//    protected $config  =	array('header'=>'条记录','prev'=>'上一页','next'=>'下一页','first'=>'第一页','last'=>'最后一页','theme'=>'%totalRow% %header% %nowPage%/%totalPage%页 %upPage% %first% %prePage% %linkPage% %nextPage% %downPage% %end%');
    protected $config  =	array('header'=>'条记录','prev'=>'上一页','next'=>'下一页','first'=>'首页','last'=>'尾页','theme'=>'%first% %upPage% %nowPage%/%totalPage% %downPage% %end%  ');
    // 默认分页变量名
    protected $varPage;
    ...
    public function show($type = 0) {
        ...
        if ($downRow <= $this->totalPages){
            $downPage="<a href='".$url.$pspan.$p."=$downRow{$idtagert}' class='prevnext add_pagebg'>".$this->config['next']."</a>";
        }else{
            $downPage="<a href='javascript:void(0);' class='prevnext delcolor'>".$this->config['next']."</a>";
        }
        if ($upRow>0){
            $upPage="<a href='".$url.$pspan.$p."=$upRow{$idtagert}'class='prevnext add_pagebg'>".$this->config['prev']."</a>";
        }else{
        ...
        if($nowCoolPage == 1){
            ...
            $prePage = "<a href='".$url.$pspan.$p."=$preRow{$idtagert}' >上".$this->rollPage."页</a>";
            $theFirst = "<a href='".$url.$pspan.$p."=1' class='prevnext add_pagebg'>".$this->config['first']."</a>";
        }
        if($nowCoolPage == $this->coolPages){
            ...
            $nextPage = "<a href='".$url.$pspan.$p."=$nextRow{$idtagert}' >下".$this->rollPage."页</a>";
            $theEnd = "<a href='".$url.$pspan.$p."=$theEndRow{$idtagert}' class='prevnext add_pagebg'>".$this->config['last']."</a>";
        }
        // 1 2 3 4 5
        $linkPage = "";
...
//        if($this->totalPages>1){
//            $pageStr .="<input type='text' value='' class='gopage' style='width: 30px;'/><input type='button' value='跳转' onclick='gopage()'/>";
//
//            if($type ==0){
//                $pageStr .="<script>function gopage(){var p=$('.gopage').val();window.location.href='".$url.$pspan.$p."='+p;}</script>";
//
//            }
//
//
