                ...
                if(empty($name) || true === $name || $mappingName == $name || (is_array($name) && in_array($mappingName,$name))) {
                    ...
                    if(!$return){
                        if(isset($val['as_fields']) && in_array($mappingType,array('HAS_ONE','BELONGS_TO')) ) {
                            ...
                            /**
                             * @Author LiuKeTao
                             * 增加了根据as_fields不存在时，根据mapping_fields映射，
                             * mapping_fields不存在时，根据getDbFields映射
                             */
                            if($mappingType=='HAS_ONE'){
                                $fields = isset($val['mapping_fields'])?explode(',',$val['mapping_fields']):$model->getDbFields();
                                foreach ($fields as $field){
                                    if(strpos($field,':')) {
                                        list($name,$nick) = explode(':',$field);
                                        $result[$nick]  =  $relationData[$name];
                            ...
                            case 'HAS_ONE':
                                switch (strtoupper($opType)){
                                    case 'ADD': // 增加关联数据
                                        ...
                                        break;
                                    case 'SAVE':    // 更新关联数据
                                        if($model->where($mappingCondition)->count()){
