...
ui = window.ui ||{
	...
	quicklogin:function(){
		ui.box.load( U('Website/public/quick_login') ,{title:'快速登录'});
	},
	
	sendmessage:function(touid){
		touid = touid || '';
		ui.box.load(U('Website/Message/post',['touid='+touid]), {title:'发私信'});
	},
	...
	emotions:function(o){
		...
		var _this = this;
		if(!this.emotdata){
			$.post( U('Website/user/emotions'),{target:$(this).attr('target_set')} ,function(txt){
				txt = eval('('+txt+')');
	...
	countNew:function(){
		$.getJSON( U('Website/User/countNew') ,function(txt){
			if(txt.total!="0"){
	...
	getarea:function(prefix,init_style,init_p,init_c){
		...
		document.write(html);
		// _PUBLIC_+'/js/area.js'
		$.getJSON(U('Website/Public/getArea'), function(json){
			json = json.provinces;
